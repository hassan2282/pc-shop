...
function EmailConfirm() {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await apiClient.post('/auth/email-verify', formData);
      const { data, status } = res;
      if (data === false) {
        toast.error('کد وارد شده صحیح نیست')
      } else {
        navigate('/store/set-new-password', { replace: true });
  ...
  useEffect(() => {
    if (timeLeft <= 0) {
      const deleteExpiredCode = async () => {
        try {
          const res = await apiClient.post('/auth/rm-code', { 'email': clientEmail });
          const { data, status } = res;
          console.log(data);
      ...
      <div className="w-full h-full flex flex-row z-20" dir="rtl">
        <div className="h-full w-full z-20 flex justify-center items-center">
          <motion.div
              ...
              <form className="flex flex-col justify-center items-center w-[95%] h-full p-3">
                <div className="flex flex-col space-y-5 w-full h-full justify-center items-center ">
                  <motion.div
                    ...
                    className="w-full">
                    <div className="text-zinc-800 flex flex-row w-full p-1">
                      <span className="flex basis-1/2 text-zinc-600">کد تایید</span>
                      <motion.div
                        initial={{
                          opacity: 0,
                          scale: 0.8,
                        ...
                        className="flex flex-row basis-1/2 text-black/60 text-xl justify-start items-center gap-2" dir="ltr">
                    ...
                    <div className="grid grid-cols-4 justify-around gap-2" dir="ltr">
                      {otp.map((digit, index) => (
                        <input
                          ...
                          className="rounded-2xl p-2 bg-black/50 text-white text-center text-xl font-bold border border-black/30
                    ...
                    <motion.div
                      ...
                      <button
                        ...
                        disabled={timeLeft <= 0}
                        className={`${timeLeft <= 0 && 'hidden'} d-flex w-full justify-center items-center 
                         rounded-xl shadow-sm shadow-zinc-500 
                         p-2 cursor-pointer hover:scale-102 active:scale-95 duration-300 hover:bg-white/60
                         border-t-white border-t-1 bg-[#00C8D1]/15`}
                      >
                        {isLoading ? (
                          <AiOutlineLoading
                  ...
                  <div className="grid grid-cols-1 w-full">
                    ...
                    <motion.div
                      ...
                      className="flex w-full justify-center items-center">
                      <Link className="d-flex w-full justify-center items-center
            ...
            <FcLock size={'21rem'} className="max-lg:hidden min-lg:basis-1/2 flex 
              hue-rotate-140 rounded-l-2xl object-cover" />
          </motion.div>
        </div>
      </div>
    </div>
